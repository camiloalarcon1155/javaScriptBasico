...
class Estrella{ /* inicia con mayuscula las clases de un objeto */
    constructor(nombre, foto, vida, tipo, fotoMapa/* ,  x=65, y=413 */ ) {/* que voy a construir, todas las propiedades que van a llevar mis objetos */
        ...
        this.ataques = []
        this.tipo = tipo
        this.ancho = 0.047*anchoDelMapa//getBoundingClientRect().width
        this.alto = 0.047*anchoDelMapa//0.084*anchoDelMapa
...
function secuenciaAtaque() {
    ...
    botones.forEach((boton) => { /* por cada boton que exista en el erreglo de botones, has esto: agregarle el evento de click y validar cual es el valor que estamos seleecionando nsotros para jugar con el   */
        boton.addEventListener('click', (e) => { /* la e significa el evento mismo. Cuando le de clic me va a regresar cual es el elemento que esta sucediendo en ese momento. Nos permite entrar a las propiedades del objeto y eeste caso con target entramos a textContent para hallar el emogi de cada elementoy ver que hacemos con el  */
            ...
            ataqueAleatEnemigo() /* lo metemos al flujo para que inmediatamente se de un click, tambien aparezca el poder del enemigo y no que aparezca el poder del enemigo primero */
        })
...
function ataqueAleatEnemigo() {
    ...
    let ataqueAleatorio = aleatorio(0, ataquesEstrellaEnemigo.length - 1) /* le restamos 3 a la variable, para que me de una iteracion entre 0 y 4*/
...
//hacer que no se llene el contenedor mas de  una vez (funcion mostrarPoderes)
//hacer que personajes queden igualmente disminuyan con el mapa
//botones no sirven en celular
